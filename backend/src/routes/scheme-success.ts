...
import { SchemeSuccessService } from '../services/schemes/scheme-success.service.js';
...
router.get('/metrics', requireAuth, validate(businessQuerySchema, 'query'), async (req, res, next) => {
  try {
    ...
    const metrics = await SchemeSuccessService.getApplicationMetrics(businessId as string);
    success(res, metrics);
  } catch (error) {
    next(error);
  }
