...
import { CustomerLoyaltyService } from '../services/customer/loyalty.service.js';
...
router.get('/loyalty', validate(businessQuerySchema, 'query'), async (req, res, next) => {
  try {
    const { businessId } = req.query;
    await ContextService.resolve({ userId: req.user!.userId, requestedBusinessId: businessId as string });

    const metrics = await CustomerLoyaltyService.getLoyaltyMetrics(businessId as string);
    success(res, metrics);
  } catch (error) {
    next(error);
  }
