...
import { DataGovernanceService } from '../services/security/data-governance.service.js';

const router = Router();

router.get('/', requireAuth, async (req: Request, res: Response, next: NextFunction) => {
  res.json({ success: true, message: 'Data Governance module loaded' });
});
...
router.post('/analyze', requireAuth, async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    const data = await DataGovernanceService.analyze(businessId);
