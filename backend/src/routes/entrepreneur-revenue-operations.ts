...
import { RevenueOperationsService } from '../services/revenue/revenue-operations.service.js';
...
router.get('/metrics', requireAuth, validate(businessQuerySchema, 'query'), async (req, res, next) => {
  try {
    ...
    const metrics = await RevenueOperationsService.getMetrics(businessId as string);
    success(res, metrics);
  } catch (error) {
    next(error);
  }
