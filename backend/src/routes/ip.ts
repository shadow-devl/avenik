...
import { IntellectualPropertyService } from '../services/legal/ip.service.js';

const router = Router();

router.get('/', requireAuth, async (req: Request, res: Response, next: NextFunction) => {
  res.json({ success: true, message: 'IP module loaded' });
});
...
router.post('/analyze', requireAuth, async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    const data = await IntellectualPropertyService.analyze(businessId);
