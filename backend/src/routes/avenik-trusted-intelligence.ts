...
import { TrustedIntelligenceService } from '../services/trust/trusted-intelligence.service.js';
...
router.get('/metrics', requireAuth, validate(businessQuerySchema, 'query'), async (req, res, next) => {
  try {
    ...
    const metrics = await TrustedIntelligenceService.getMetrics(businessId as string);
    success(res, metrics);
  } catch (error) {
    next(error);
  }
...
export const trustedIntelligenceRouter = router;
