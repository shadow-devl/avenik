...
export default function CommunityPage() {
  ...
  async function fetchMetrics() {
    try {
      ...
      if (ctx.success && ctx.data.business) {
        ...
        const res = await apiGet<any>(`/api/generic-intelligence/metrics?businessId=${bid}&domain=community`);
        if (res.success) {
          setMetrics(res.data);
        }
      }
  ...
  if (status === "loading" || loading) return <div className="p-8 text-slate-400">Loading Community...</div>;
