...
export default function InternationalPage() {
  ...
  async function fetchMetrics() {
    try {
      ...
      if (ctx.success && ctx.data.business) {
        ...
        const res = await apiGet<any>(`/api/generic-intelligence/metrics?businessId=${bid}&domain=international`);
        if (res.success) {
          setMetrics(res.data);
        }
      }
  ...
  if (status === "loading" || loading) return <div className="p-8 text-slate-400">Loading International...</div>;
    ...
    <div className="min-h-screen bg-slate-950 pb-12">
      ...
      <main className="mx-auto max-w-6xl px-6 py-12">
        <div className="flex items-center gap-3 mb-8">
          ...
          <div>
            <h1 className="text-3xl font-bold text-white tracking-tight">International</h1>
