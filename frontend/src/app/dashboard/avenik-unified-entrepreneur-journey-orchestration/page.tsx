...
export default function AvenikUnifiedEntrepreneurJourneyOrchestrationPage() {
  ...
  async function fetchMetrics() {
    try {
      ...
      if (ctx.success && ctx.data.business) {
        ...
        const res = await apiGet<any>(`/api/generic-intelligence/metrics?businessId=${bid}&domain=avenik-unified-entrepreneur-journey-orchestration`);
        if (res.success) {
          setMetrics(res.data);
        }
      }
  ...
  if (status === "loading" || loading) return <div className="p-8 text-slate-400">Loading Avenik Unified Entrepreneur Journey Orchestration...</div>;
    ...
    <div className="min-h-screen bg-slate-950 pb-12">
        ...
        <div className="mx-auto flex max-w-7xl items-center justify-between px-6 py-4">
          <div className="flex items-center gap-8">
            ...
            <span className="rounded-full bg-slate-800 px-3 py-1 text-xs font-medium text-slate-300">
              Avenik Unified Entrepreneur Journey Orchestration
            </span>
          </div>
      ...
      <main className="mx-auto max-w-6xl px-6 py-12">
        <div className="flex items-center gap-3 mb-8">
          ...
          <div>
            <h1 className="text-3xl font-bold text-white tracking-tight">Avenik Unified Entrepreneur Journey Orchestration</h1>
